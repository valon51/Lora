...
void LoRa_readReg(LoRa* _LoRa, uint8_t* address, uint16_t r_length, uint8_t* output, uint16_t w_length){
	...
	while(!__HAL_SPI_GET_FLAG(_LoRa->hSPIx, SPI_FLAG_TXE));
	HAL_SPI_Transmit_DMA(_LoRa->hSPIx, address, r_length);
	HAL_SPI_Receive_DMA(_LoRa->hSPIx, output, w_length);
	HAL_GPIO_WritePin(_LoRa->CS_port, _LoRa->CS_pin, GPIO_PIN_SET);
}
void LoRa_readReg_Nom(LoRa* _LoRa, uint8_t* address, uint16_t r_length, uint8_t* output, uint16_t w_length){
	HAL_GPIO_WritePin(_LoRa->CS_port, _LoRa->CS_pin, GPIO_PIN_RESET);
...
void LoRa_writeReg_Nom(LoRa* _LoRa, uint8_t* address, uint16_t r_length, uint8_t* values, uint16_t w_length){
	HAL_GPIO_WritePin(_LoRa->CS_port, _LoRa->CS_pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit(_LoRa->hSPIx, address, r_length, TRANSMIT_TIMEOUT);
	while (HAL_SPI_GetState(_LoRa->hSPIx) != HAL_SPI_STATE_READY)
		;
...
uint8_t LoRa_read_Nom(LoRa* _LoRa, uint8_t address){
	uint8_t read_data;
	uint8_t data_addr;

	data_addr = address & 0x7F;
	LoRa_readReg_Nom(_LoRa, &data_addr, 1, &read_data, 1);
	//HAL_Delay(5);
...
void LoRa_write_Nom(LoRa* _LoRa, uint8_t address, uint8_t value){
	uint8_t data;
	uint8_t addr;

	addr = address | 0x80;
	data = value;
	LoRa_writeReg_Nom(_LoRa, &addr, 1, &data, 1);
	//HAL_Delay(5);
}
...
uint8_t LoRa_transmit_Nom(LoRa* _LoRa, uint8_t* data, uint8_t length, uint16_t timeout){
	uint8_t read;

	LoRa_write_Nom(_LoRa, RegIrqFlags, 0xFF);

	int mode = _LoRa->current_mode;
	LoRa_gotoMode(_LoRa, STNBY_MODE);
	read = LoRa_read_Nom(_LoRa, RegFiFoTxBaseAddr);
	LoRa_write_Nom(_LoRa, RegFiFoAddPtr, read);
	LoRa_write_Nom(_LoRa, RegPayloadLength, length);
	LoRa_BurstWrite(_LoRa, RegFiFo, data, length);
	LoRa_gotoMode(_LoRa, TRANSMIT_MODE);
	while(1){
		read = LoRa_read_Nom(_LoRa, RegIrqFlags);
		if((read & 0x08)!=0){
			LoRa_write_Nom(_LoRa, RegIrqFlags, 0xFF);
			LoRa_gotoMode(_LoRa, mode);
			return 1;
...
uint8_t LoRa_receive_Nom(LoRa* _LoRa, uint8_t* data, uint8_t length){
	uint8_t read;
	uint8_t number_of_bytes;
	uint8_t min = 0;
	...
	if((read & 0x40) != 0){
		LoRa_write_Nom(_LoRa, RegIrqFlags, 0xFF);
		number_of_bytes = LoRa_read(_LoRa, RegRxNbBytes);
		read = LoRa_read_Nom(_LoRa, RegFiFoRxCurrentAddr);
		LoRa_write_Nom(_LoRa, RegFiFoAddPtr, read);
		min = length >= number_of_bytes ? number_of_bytes : length;
		for(int i=0; i<min; i++)
			data[i] = LoRa_read_Nom(_LoRa, RegFiFo);
	}
	LoRa_gotoMode(_LoRa, RXCONTIN_MODE);
    return min;
}
...
uint16_t LoRa_init(LoRa* _LoRa){
	...
	if(LoRa_isvalid(_LoRa)){
		...
		// set LNA gain:
			//LoRa_write(_LoRa, RegLna, 0xc3);

			LoRa_write(_LoRa, RegLna, (LoRa_read(_LoRa, RegLna) | 0x03));
...
void LoraSetSynWord(LoRa* _LoRa, uint8_t value) {
